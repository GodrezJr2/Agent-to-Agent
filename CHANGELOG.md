...
- A2A routes/repos, agent tools, Office UI, fork README and docker-compose untouched upstream; `/office` sidebar entry intact.
- DeepSeek Web provider kept in the registry (`pFork1`).
- Codex Responses translator keeps native `web_search` passthrough and tool_choice mapping alongside upstream's new tool-name clamping and `clampResponsesCallId`.

## Verification
- Test suite vs upstream/master in the same env: 2429 passing, 0 regressions, 0 fork-only failures.

# v0.5.59-fork.1 (2026-08-30)

Synced fork engine with upstream 9router v0.5.40 → v0.5.59 (148 commits, 429 files; upstream releases v0.5.45, v0.5.50, v0.5.55, v0.5.59).

## Fork surfaces preserved
- A2A protocol routes/repos, agent card endpoint, agent tools, and the Office UI — upstream touched none of them.
- Sidebar auto-merged cleanly; the `/office` entry is intact.
- `src/lib/db/schema.js` and `src/lib/db/index.js` were not changed upstream this cycle — fork tables (`officeAgents`, `a2aTasks`, `chatMessages`) untouched.
- Fork README and `docker-compose.yml` (local build + workspaces volume) kept as-is.

## Restored after upstream dropped them
- **DeepSeek Web** provider — re-added to the registry index as a fork-only entry alongside upstream's new providers.

## Adopted from upstream
- **Qwen provider removed** (upstream dcdd4628) — removal adopted, not restored.
- Claude registry pruned to Opus 5 / Sonnet 5 / Fable 5 / Haiku 4.5; old dated 4.x entries dropped.
- Golden url/header snapshot deleted upstream (regenerates from the registry on test run) — adopted.
- New providers: Poolside, api-airforce, baidu, bazaarlink, bluesminds, kilo-gateway, llm7, morph, sambanova, tencent, zed, xquik, fish-audio, tokenrouter, alitp-intl, selfhosted-stt/tts/embedding; trae/windsurf/devin-cli present but hidden.
- Web search: Antigravity Google grounding + Xquik on `POST /v1/search`; `credentialFallback` lets ollama-search/zai-search borrow chat keys.
- Models: daily models.dev capability sync (`MODEL_CATALOG_SYNC=off` to disable); GLM-5.3-Flash, DeepSeek V4 Vision, Grok 4.5/4.6.
- Antigravity: quota-aware routing, image size→aspect-ratio suffixes, competing-client branding sanitization.
- Usage: Zed plan quota on dashboard; GPT-5.3-Codex-Spark quota windows; nested `cached_tokens` preserved; GLM multi-interval quotas.
- Stream fixes: usage on Responses terminal events, Ollama trailing NDJSON line, Claude Code session id from `x-claude-code-session-id`.
- Token Saver: configurable `headroomTimeoutMs`.
- next.config: `open` kept external (Windows file-URL fix); body-limit consolidated under `proxyClientMaxBodySize`.
- Full upstream notes in the v0.5.45 – v0.5.59 sections below.
