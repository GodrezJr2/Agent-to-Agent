...
export function OfficeCanvas({ onAgentClick }: { onAgentClick?: (agentId: string) => void }) {
  ...
  return (
    <div ref={containerRef} className="oc-root"
    ...
    >
      <canvas ref={canvasRef} className="oc-canvas" style={{ imageRendering: 'pixelated', cursor: 'grab' }} />
      {!assetsLoaded && <div className="oc-loading">Loading office…</div>}

      <div className="oc-zoom">
        <button onClick={() => handleZoomBtn(1)} title="Zoom in">+</button>
        {zoomDisplay !== null && <div className="oc-zoom-label">{zoomDisplay}%</div>}
        <button onClick={() => handleZoomBtn(-1)} title="Zoom out">−</button>
        <button onClick={handleZoomReset} title="Reset zoom">⌂</button>
      </div>

      <div className="oc-chips">
        {Array.from(storeChars.values()).map(char => (
          <button key={char.id} className={char.active ? 'on' : ''} onClick={e => { e.stopPropagation(); onAgentClick?.(char.id); }}>
            {char.name}
          </button>
        ))}
      </div>
