...
function buildPromptForSession({ body, flags, reused, prevMessageCount }) {
  ...
  let promptBody = body;
  if (flags.agentic) {
    promptBody = { ...body, messages: [{ role: "system", content: AGENTIC_SYSTEM_PROMPT }, ...messages] };
  }
...
export function resolveDeepSeekWasmPath(wasmUrl = new URL("./deepseek-pow.wasm", import.meta.url)) {
  ...
  if (!asString.startsWith("file:")) return path.join(process.cwd(), "src", "deepseek-pow.wasm");
...
export class DeepSeekWebExecutor {
  constructor(options = {}) {
