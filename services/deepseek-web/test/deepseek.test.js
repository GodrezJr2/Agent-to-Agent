...
describe("resolveDeepSeekWasmPath", () => {
  it("resolves URL-like wasm module references after string coercion", () => {
    const urlLike = {
      toString() {
        return new URL("../src/deepseek-pow.wasm", import.meta.url).toString();
      },
    };
  ...
  it("falls back to cwd src path when module URL is not file-based", () => {
    const resolved = resolveDeepSeekWasmPath("webpack://_N_E/src/deepseek-pow.wasm");

    expect(resolved).toBe(path.join(process.cwd(), "src", "deepseek-pow.wasm"));
  });
});
